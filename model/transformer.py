...
class CDRB(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.,
        ...
        self.ffn = SGFN(in_features=dim, hidden_features=mlp_hidden_dim, out_features=dim, act_layer=act_layer)
        self.cdim1 = CDIM(dim)
        self.cdim2 = CDIM(dim)
    ...
    def forward(self, x, cdp, x_size):
        H , W = x_size

        res = x

        x = x + self.drop_path(self.attn(self.cdim1(self.norm1(x), cdp), H, W))
        x = x + self.drop_path(self.ffn(self.cdim2(self.norm2(x), cdp), H, W))
...
class CDFormer_SR(nn.Module):
    ...
    def __init__(self,
        ...
        self.norm2 = norm_layer(curr_dim)
    ...
    def forward_features(self, x, cdp):
        _, _, H, W = x.shape
        x_size = [H, W]
        x = rearrange(x, "b c h w -> b (h w) c")
        x = self.norm1(x)
        for layer in self.layers:
            x = layer(x, x_size, cdp)
        x = self.norm2(x)
        x = rearrange(x, "b (h w) c -> b c h w", h=H, w=W)
    ...
    def forward(self, x, cdp):
        ...
        """
        x = self.conv_first(x)
        x = self.conv_after_body(self.forward_features(x, cdp)) + x
